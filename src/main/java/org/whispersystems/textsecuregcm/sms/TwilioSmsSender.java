...
public class TwilioSmsSender {
      ...
      throws IOException, TwilioRestException
  {
	...
	public void deliverSmsInvitation(String destination,
			String invitation) throws IOException, TwilioRestException {
		TwilioRestClient client = new TwilioRestClient(accountId, accountToken);
		MessageFactory messageFactory = client.getAccount().getMessageFactory();
		List<NameValuePair> messageParams = new LinkedList<>();
		messageParams.add(new BasicNameValuePair("To", destination));
	...
	public void deliverVoxVerification(String destination,
		...
		callParams.put("Url", "https://" + localDomain
				+ "/v1/accounts/voice/twiml/" + verificationCode);
